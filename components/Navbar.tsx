...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            <Link to="/admin" className={`hidden md:block px-4 py-2 rounded-lg font-bold text-xs transition ${isAdminPath ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
              Admin
            </Link>
            ...
            {user ? (
              ...
              <button 
                ...
                className="hidden sm:block text-xs font-bold text-slate-800 bg-gray-50 hover:bg-gray-100 px-4 py-2 rounded-xl border border-gray-100 transition"
              >
                Sign In
              </button>
            )}
            ...
            <button 
              ...
              className="md:hidden p-2 rounded-xl text-slate-800 hover:bg-gray-100 transition"
              aria-label="Menu"
            >
              {isMenuOpen ? (
      ...
      {/* Mobile Menu Overlay */}
      <div className={`md:hidden absolute top-16 left-0 right-0 bg-white border-b border-gray-100 shadow-xl transition-all duration-300 origin-top transform ${isMenuOpen ? 'scale-y-100 opacity-100 visible' : 'scale-y-0 opacity-0 invisible'}`}>
        <div className="px-6 py-8 space-y-6">
          <div className="flex flex-col space-y-4">
            <Link to="/" className={`text-lg font-bold transition ${isActive('/') ? 'text-blue-600' : 'text-slate-800 hover:text-blue-600'}`}>Home</Link>
            <Link to="/shop" className={`text-lg font-bold transition ${isActive('/shop') ? 'text-blue-600' : 'text-slate-800 hover:text-blue-600'}`}>Shop</Link>
            <Link to="/about" className={`text-lg font-bold transition ${isActive('/about') ? 'text-blue-600' : 'text-slate-800 hover:text-blue-600'}`}>About</Link>
            <Link to="/contact" className={`text-lg font-bold transition ${isActive('/contact') ? 'text-blue-600' : 'text-slate-800 hover:text-blue-600'}`}>Contact</Link>
          </div>
          ...
          <div className="pt-6 border-t border-gray-100 space-y-4">
            {user ? (
               <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden border border-white">
                    <img src={user.avatar} alt={user.name} />
                  </div>
                  <div>
            ...
            <Link 
              to="/admin" 
              className={`block w-full text-center py-4 rounded-xl font-bold transition ${isAdminPath ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
            >
              Admin Panel
            </Link>
          </div>
