...
const Home: React.FC<HomeProps> = ({ products, addToCart, wishlist, onToggleWishlist }) => {
  ...
  return (
    <div className="space-y-16 pb-20">
      {/* Hero Section */}
      <section className="relative h-[600px] bg-gray-100 dark:bg-slate-900 overflow-hidden">
        <div 
      ...
      <section className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="flex items-center space-x-4 p-6 bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-md transition cursor-default">
            <div className="w-12 h-12 bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-2xl rounded-full">{f.icon}</div>
            <div>
              <h4 className="font-bold text-slate-800 dark:text-white">{f.title}</h4>
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Shop by Category</h2>
          <button className="text-blue-600 dark:text-blue-400 font-medium hover:underline text-sm">View All</button>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {CATEGORIES.map(cat => (
            <div key={cat.id} className="group bg-white dark:bg-slate-900 p-8 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all cursor-pointer text-center space-y-4">
              <div className={`w-16 h-16 ${cat.color} dark:bg-slate-800 mx-auto rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform`}>
                {cat.icon}
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Trending Now</h2>
        </div>
